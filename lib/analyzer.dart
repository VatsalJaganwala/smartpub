...
class DependencyAnalyzer {
      ...
      await for (final FileSystemEntity entity in directory.list(recursive: true)) {
        if (entity is File && entity.path.endsWith(FileConfig.dartExtension)) {
          ...
          for (final match in matches) {
            ...
            if (relativePath.startsWith('lib${path.separator}') || relativePath.startsWith('lib/')) {
              usage.usedInLib = true;
            } else if (relativePath.startsWith('test${path.separator}') || relativePath.startsWith('test/')) {
              usage.usedInTest = true;
            } else if (relativePath.startsWith('bin${path.separator}') || relativePath.startsWith('bin/')) {
              usage.usedInBin = true;
            } else if (relativePath.startsWith('tool${path.separator}') || relativePath.startsWith('tool/')) {
  ...
  DependencyStatus _determineDependencyStatus(String packageName, PackageUsage? usage) {
    if (usage == null) {
  ...
  List<DuplicateDependency> _findDuplicates(Map<String, dynamic> deps, Map<String, dynamic> devDeps, Map<String, PackageUsage> usageMap) {
  ...
  DependencySection _getRecommendedSection(String packageName, PackageUsage? usage) {
    if (usage == null) {
...
class AnalysisResult {
  ...
  List<DependencyInfo> get testOnlyDependencies => getDependenciesByStatus(DependencyStatus.testOnly);
  
  /// Get unused dependencies
  List<DependencyInfo> get unusedDependencies => getDependenciesByStatus(DependencyStatus.unused);
  
  /// Check if there are any issues
  bool get hasIssues => testOnlyDependencies.isNotEmpty || unusedDependencies.isNotEmpty || duplicates.isNotEmpty;
}
