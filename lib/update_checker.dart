...
class UpdateChecker {
  ...
  static bool _isNewerVersion(String version1, String version2) {
    final v1Parts = version1.split('.').map(int.tryParse).where((int? v) => v != null).cast<int>().toList();
    final v2Parts = version2.split('.').map(int.tryParse).where((int? v) => v != null).cast<int>().toList();
